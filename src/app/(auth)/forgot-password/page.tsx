...
export default function ForgotPasswordPage() {
  ...
  const forgotMutation = useMutation({
    ...
    onSuccess: (data) => {
      setSuccessMsg(data?.message || "OTP has been sent to your email. Redirecting...");
      setTimeout(() => {
        router.push(`/reset-password?email=${encodeURIComponent(email)}`);
      }, 2000);
    },
    onError: (error: any) => {
      setErrorMsg(error?.response?.data?.email?.[0] || error?.response?.data?.detail || "Failed to request password reset. Please try again.");
    }
  });
      ...
      <div className="relative z-10 w-full max-w-[900px] backdrop-blur-2xl bg-white/10 border border-white/20 shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] rounded-[2rem] overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="flex-[1.2] p-8 md:p-12 bg-white/5">
          <div className="w-full max-w-sm mx-auto">
            ...
            <p className="text-white/60 text-sm mb-8">Enter your registered email address to receive a 6-digit OTP code.</p>
